...
int32_t GenC_gen_instr_encoding_def(
  ...
  for (uint32_t i = 0; i < n_forms; ++i) {
    ...
    GenC_gen_instr_form(context, ident_array, buffer, &forms[i]);
...
void GenC_gen_instr_exec(Generator *generator, const Machine *machine) {
  ...
  Array *def_buffer = CGenerator_getOutputBuffer((CGenerator *) generator, GenC_definitions);
...
void GenC_gen_jump_table_def(
  ...
  for (uint32_t i = 0; i < state_count; i++) {
    ...
    if (form_ndx != 0) {
      sprintf(temp2_buffer, ENCODING_NAME_FMT, ctx_ident_real(instr->name), form_ndx - 1);
      sprintf(
    ...
    case enum_Variable: {
      switch (variable->type) {
        ...
        case enum_MemItem: {
          ...
          return sprintf(
              buffer, "((%s->value>>%d)&UINT_N_MAX(%d))", ctx_ident_real(ident), item->start, width
          );
        }
      }
    }
...
void type_to_val(
    const ParseContext *context, const Array *ident_array, const Identifier *ident, char_t *buffer
) {
...
int32_t expr_to_val(
  ...
  if (expr->type < RECURSIVE_OP_MAX) {
    if (expr->lhs) {
      expr_to_val(context, ident_array, expr->lhs, pattern, buffer);
      strcpy(temp_buffer1, buffer);
    }
    if (expr->rhs) {
      expr_to_val(context, ident_array, expr->rhs, pattern, buffer);
  ...
  switch (expr->type) {
    ...
    case AS_ID: {
      eval_to_val(context, ident_array, expr->rhs, temp_buffer2, pattern);
...
int32_t GenC_gen_mapping_item(
  ...
  if (bu < bit_field->upper) {
    ...
    GenC_gen_mapping_item(context, ident_array, buffer, items, &upper_bf, pattern, temp_buffer);
  }
  return (int32_t) (Array_length(buffer) - pre_len);
}
  ...
  switch (layout->type) {
    ...
    case enum_MappingItems: {
      ...
      for (uint32_t i = 0; i < width; i += 64) {
        ...
        GenC_gen_mapping_item(context, ident_array, buffer, items, &bf, pattern, temp_buffer);
...
int32_t GenC_gen_instr_part(
  ...
  if (condition) {
    ...
    push_string(temp_buffer);
    expr_to_val(context, ident_array, condition->expr, form->pattern, temp_buffer);
    push_string(temp_buffer);
    push_string(") {\n");
  } else {
    sprintf(temp_buffer, "  /* %s */ {\n", ctx_ident_real(name));
  ...
  GenC_gen_layout(context, ident_array, buffer, layout, width, form->pattern, temp_buffer);
  sprintf(temp_buffer, "  }\n", width / 8);
  push_string(temp_buffer);
  return 0;
}
...
int32_t GenC_gen_instr_form(
  ...
  if (form->check) { GenC_gen_form_check(context, ident_array, buffer, form, temp_buffer); }
  for (uint32_t i = 0; i < n_parts; i++) {
    GenC_gen_instr_part(context, ident_array, buffer, form, &parts[i], temp_buffer);
  }
  return (int32_t) (Array_length(buffer) - pre_len);
}
