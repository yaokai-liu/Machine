...
uint32_t Tokenizer_parse(Tokenizer *tokenizer, Terminal *terminal);
...
#define currentToken (tokenizer->terminals[tokenizer->n_tokens])
uint32_t Tokenizer_next(Tokenizer *tokenizer, Terminal *const terminal) {
  if (tokenizer->terminals) {
    if (currentToken.type == enum_PLACE_HOLDER) {
      // TODO:
    }
    tokenizer->allocator->memcpy(terminal, &currentToken, sizeof(Terminal));
    return 0;
  }
  uint32_t cost = 0;
  ...
  if (terminal->type == enum_IDENTIFIER) {
    if (Tokenizer_hasMacro(tokenizer, (Identifier *)terminal->value)) {
      Tokenizer_parse(tokenizer, terminal);
      // TODO:
    }
  }
