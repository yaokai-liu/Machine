...
START_TEST(test_NUMBER_r16) {
  ...
  ck_assert_uint_eq(terminals[0].type, Machine_TOKEN_NUMBER);
...
START_TEST(test_NUMBER_r16_prime) {
  const char_t *string = "0x123'45678'9abcd'ef0";
  uint32_t cost = 0, n_tokens = 0;
  uint32_t lineno = 0, column = 0;
  const Terminal *terminals = tokenize(string, &cost, &n_tokens, &lineno, &column, &STDAllocator);
  ck_assert_uint_eq(cost, lenof("0x123'45678'9abcd'ef0"));
  ck_assert_uint_eq(n_tokens, 2);
  ck_assert_ptr_ne(terminals, nullptr);
  ck_assert_uint_eq(terminals[0].type, Machine_TOKEN_NUMBER);
  ck_assert_uint_eq(terminals[0].lineno, 0);
  ck_assert_uint_eq(terminals[0].column, 0);
  ck_assert_uint_eq(terminals[0].length, lenof("0x123'45678'9abcd'ef0"));
  ck_assert_uint_eq((uint64_t) terminals[0].value, 0x123456789abcdef0LL);
  ck_assert_str_eq(get_name(terminals[0].type), "NUMBER");
  ck_assert_uint_eq(terminals[1].type, Machine_TOKEN_TERMINATOR);
  ck_assert_ptr_eq(terminals[1].value, nullptr);
  ck_assert_uint_eq(terminals[1].lineno, 0);
  ck_assert_uint_eq(terminals[1].column, lenof("0x123'45678'9abcd'ef0"));
  ck_assert_uint_eq(terminals[1].length, 0);
  ck_assert_str_eq(get_name(terminals[1].type), "TERMINATOR");
  STDAllocator.free((void *) terminals);
  ck_assert_uint_eq(lineno, 0);
  ck_assert_uint_eq(column, lenof("0x123'45678'9abcd'ef0"));
}
END_TEST
...
START_TEST(test_NUMBER_r10_prime) {
  const char_t *string = "12345'67890";
  uint32_t cost = 0, n_tokens = 0;
  uint32_t lineno = 0, column = 0;
  const Terminal *terminals = tokenize(string, &cost, &n_tokens, &lineno, &column, &STDAllocator);
  ck_assert_uint_eq(cost, lenof("12345'67890"));
  ck_assert_uint_eq(n_tokens, 2);
  ck_assert_ptr_ne(terminals, nullptr);
  ck_assert_uint_eq(terminals[0].type, Machine_TOKEN_NUMBER);
  ck_assert_uint_eq(terminals[0].lineno, 0);
  ck_assert_uint_eq(terminals[0].column, 0);
  ck_assert_uint_eq(terminals[0].length, lenof("12345'67890"));
  ck_assert_uint_eq((uint64_t) terminals[0].value, 1234567890);
  ck_assert_str_eq(get_name(terminals[0].type), "NUMBER");
  ck_assert_uint_eq(terminals[1].type, Machine_TOKEN_TERMINATOR);
  ck_assert_ptr_eq(terminals[1].value, nullptr);
  ck_assert_uint_eq(terminals[1].lineno, 0);
  ck_assert_uint_eq(terminals[1].column, lenof("12345'67890"));
  ck_assert_uint_eq(terminals[1].length, 0);
  ck_assert_str_eq(get_name(terminals[1].type), "TERMINATOR");
  STDAllocator.free((void *) terminals);
  ck_assert_uint_eq(lineno, 0);
  ck_assert_uint_eq(column, lenof("12345'67890"));
}
END_TEST
...
START_TEST(test_NUMBER_r8_prime) {
  const char_t *string = "0o1234'5670";
  uint32_t cost = 0, n_tokens = 0;
  uint32_t lineno = 0, column = 0;
  const Terminal *terminals = tokenize(string, &cost, &n_tokens, &lineno, &column, &STDAllocator);
  ck_assert_uint_eq(cost, lenof("0o1234'5670"));
  ck_assert_uint_eq(n_tokens, 2);
  ck_assert_ptr_ne(terminals, nullptr);
  ck_assert_uint_eq(terminals[0].type, Machine_TOKEN_NUMBER);
  ck_assert_uint_eq((uint64_t) terminals[0].value, 012345670);
  ck_assert_uint_eq(terminals[0].lineno, 0);
  ck_assert_uint_eq(terminals[0].column, 0);
  ck_assert_uint_eq(terminals[0].length, lenof("0o1234'5670"));
  ck_assert_str_eq(get_name(terminals[0].type), "NUMBER");
  ck_assert_uint_eq(terminals[1].type, Machine_TOKEN_TERMINATOR);
  ck_assert_ptr_eq(terminals[1].value, nullptr);
  ck_assert_str_eq(get_name(terminals[1].type), "TERMINATOR");
  ck_assert_uint_eq(terminals[1].lineno, 0);
  ck_assert_uint_eq(terminals[1].column, lenof("0o1234'5670"));
  ck_assert_uint_eq(terminals[1].length, 0);
  STDAllocator.free((void *) terminals);
  ck_assert_uint_eq(lineno, 0);
  ck_assert_uint_eq(column, lenof("0o1234'5670"));
}
END_TEST
...
START_TEST(test_NUMBER_r2_prime) {
  const char_t *string = "0b1010'10101";
  uint32_t cost = 0, n_tokens = 0;
  uint32_t lineno = 0, column = 0;
  const Terminal *terminals = tokenize(string, &cost, &n_tokens, &lineno, &column, &STDAllocator);
  ck_assert_uint_eq(cost, lenof("0b1010'10101"));
  ck_assert_uint_eq(n_tokens, 2);
  ck_assert_ptr_ne(terminals, nullptr);
  ck_assert_uint_eq(terminals[0].type, Machine_TOKEN_NUMBER);
  ck_assert_uint_eq((uint64_t) terminals[0].value, 0b101010101);
  ck_assert_uint_eq(terminals[0].lineno, 0);
  ck_assert_uint_eq(terminals[0].column, 0);
  ck_assert_uint_eq(terminals[0].length, lenof("0b1010'10101"));
  ck_assert_str_eq(get_name(terminals[0].type), "NUMBER");
  ck_assert_uint_eq(terminals[1].type, Machine_TOKEN_TERMINATOR);
  ck_assert_ptr_eq(terminals[1].value, nullptr);
  ck_assert_str_eq(get_name(terminals[1].type), "TERMINATOR");
  ck_assert_uint_eq(terminals[1].lineno, 0);
  ck_assert_uint_eq(terminals[1].column, lenof("0b1010'10101"));
  ck_assert_uint_eq(terminals[1].length, 0);
  STDAllocator.free((void *) terminals);
  ck_assert_uint_eq(lineno, 0);
  ck_assert_uint_eq(column, lenof("0b1010'10101"));
}
END_TEST
...
Suite *number_suite() {
  ...
  tcase_add_test(tc_numbers, test_NUMBER_r16_prime);
  tcase_add_test(tc_numbers, test_NUMBER_r10_prime);
  tcase_add_test(tc_numbers, test_NUMBER_r8_prime);
  tcase_add_test(tc_numbers, test_NUMBER_r2_prime);
